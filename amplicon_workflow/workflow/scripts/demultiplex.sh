 ...
 dorado demux \
     ...
     --output-dir ${snakemake_output[output_dir]}/.demux_tmp \
     ${snakemake_input}

# Move the demultiplexed FASTQ files to the output directory and remove the temporary directory
find ${snakemake_output[output_dir]}/.demux_tmp -name "*.fastq" -exec mv {} ${snakemake_output[output_dir]}/ \;
rm -rf ${snakemake_output[output_dir]}/.demux_tmp
...
for file in "$output_dir"/*.fastq.gz; do
    ...
    if [[ "$base_name" =~ _barcode[0-9]+ ]]; then
        new_name=$(echo "$base_name" | sed -E 's/^.*(barcode[0-9]+).*\.fastq\.gz$/\1.fastq.gz/')
    elif [[ "$base_name" =~ _unclassified ]]; then
