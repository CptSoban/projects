...
rule filter_sequences_with_asterisk:
    ...
    output:
        cleaned_aa = "results/{run_id}/cleaned.aa", 
    run:
        filter_sequences_with_asterisk(input[0], output[0])


#Download the latest InterProScan data
rule download_interpro_data:
    output:
        interpro_data = directory(f"resources/interproscan-{config['interproscan_version']}/data")
    shell:
        f"curl -O --output-dir resources/ http://ftp.ebi.ac.uk/pub/software/unix/iprscan/5/{config['interproscan_version']}/alt/interproscan-data-{config['interproscan_version']}.tar.gz && "
        f"tar -pxzf resources/interproscan-data-{config['interproscan_version']}.tar.gz -C resources/ && "
        f"rm -r resources/interproscan-data-{config['interproscan_version']}.tar.gz"

# Pull the latest InterProScan Singularity container
rule pull_interpro_container:
    output:
        f"resources/interproscan_{config['interproscan_version']}.sif"
    shell:
        f"apptainer pull --force --dir resources/ docker://interpro/interproscan:{config['interproscan_version']}"

# Run InterProScan
rule interproscan_run:
    input:
        cleaned_aa = "results/{run_id}/cleaned.aa",
        interpro_data = f"resources/interproscan-{config['interproscan_version']}/data",
        container_file = f"resources/interproscan_{config['interproscan_version']}.sif"
 
    output:
        interpro_gff = "results/{run_id}/interproscan/{run_id}.gff3",

    params:
        interpro_output = "results/{run_id}/interproscan/{run_id}",
        temp_dir = directory("results/{run_id}/interproscan/temp"),
